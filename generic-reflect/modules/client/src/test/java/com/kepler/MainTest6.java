...
public class MainTest6 {
	...
	public void test1WithNoneClass() throws Throwable {
		...
		event_2.put("level", event_2_level);
		events.put(time_2, event_2);
		Map<Object, Object> response = Map.class.cast(this.generic.invoke(new Service("com.kepler.TestApp6", "0.0.1-generic"), "test1", null, user));
		for (Object each : Collection.class.cast(Map.class.cast(response.get("dept")).get("levels"))) {
			Map<Object, Object> level = Map.class.cast(each);
			int[] ids = int[].class.cast(level.remove("ids"));
			Assert.assertTrue(ids[0] == 1);
	...
	public void test3WithNoneClass() throws Throwable {
		...
		event_2.put("level", event_2_level);
		events.put(time_2, event_2);
		Map<Object, Object> response = Map.class.cast(this.generic.invoke(new Service("com.kepler.TestApp6", "0.0.1-generic"), "test2", null, user));
		for (Object each : Collection.class.cast(Map.class.cast(response.get("dept")).get("levels"))) {
			Map<Object, Object> level = Map.class.cast(each);
			int[] ids = int[].class.cast(level.remove("ids"));
			Assert.assertTrue(ids[0] == 1);
